# Recursive Inundation Model Algorithm v1
...
class InundationModel:
    ...
    their heights by 1 unit until self.grid.maxHeight
    """
    def tick(self):
        self.stepCount += 1
        for position in self.blueSpotPositions:
            row, column = position
            if self.grid.positions[row][column] < self.grid.maxHeight:
                self.grid.positions[row][column] += 0.1
    ...
    def simulate(self, tickSpeed: float = 1.0) -> None:
        while len(self.blueSpotPositions) < self.grid.size ** 2:
            print("Ticked:")
